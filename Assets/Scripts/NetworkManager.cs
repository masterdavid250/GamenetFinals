...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    public ShipExplorationLineup[] explorationChallengers;
    public ShipRacingLineup[] shipRacingChallengers;
    ...
    public void OnStartGameButtonClicked()
    {
        if (PhotonNetwork.CurrentRoom.CustomProperties.ContainsKey("gm"))
        {
            if (PhotonNetwork.CurrentRoom.CustomProperties.ContainsValue("rc"))
            {
                ...
                PhotonNetwork.LoadLevel("ShipExplorationScene"); 
            }
            else if (PhotonNetwork.CurrentRoom.CustomProperties.ContainsValue("dr"))
            {
                //PhotonNetwork.LoadLevel("DeathRaceScene");
                PhotonNetwork.LoadLevel("ShipRacingScene"); 
            }
        }
    }
    #endregion
    ...
    public override void OnJoinedRoom()
    {
        ...
        if (PhotonNetwork.CurrentRoom.CustomProperties.TryGetValue("gm", out gameModeName))
        {
            ...
            if (PhotonNetwork.CurrentRoom.CustomProperties.ContainsValue("rc"))
            {
                GameModeText.text = "Exploration Mode";

                for (int i = 0; i < CarPool.Length; i++)
                {
                    CarPool[i].transform.Find("PlayerName").GetComponent<Text>().text = explorationChallengers[i].playerName;
            ...
            else if (PhotonNetwork.CurrentRoom.CustomProperties.ContainsValue("dr"))
            {
                ...
                for (int i = 0; i < CarPool.Length; i++)
                {
                    CarPool[i].transform.Find("PlayerName").GetComponent<Text>().text = shipRacingChallengers[i].playerName;
                    CarPool[i].transform.Find("PlayerProperty").GetComponent<Text>().text = shipRacingChallengers[i].weaponName; 
                }
            }
        }
